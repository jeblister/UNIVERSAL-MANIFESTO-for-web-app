# UNIVERSAL-MANIFESTO-for-web-app
As part of change in the JavaScript world. I write this manifesto to share with the community the rules of Universal (Isomorphic) application.


It provides a set of rules that all JS communities and groups, no matter what stacks they shoose, should abide by, uniting JS Universal application as a movement. They are:

1. Our law
> Everything that happens in your app is an “action”. These can be caused by users, browser events, or server events. Doesn’t matter. Everything that changes something in your app does it via an “action”.

2. Our wild
> You have one giant state object that represents all the state in your app. These are not special Models, or Collections, it’s just friggin’ objects, arrays, and primitives. 

3. Our aim
>  ...

4. Our method
> You write “reducers”: (state,action) => state for everything that changes state. That’s exactly what we want to do in response to actions. We get the starting state, the current action, and we return the new state.

5. Our primary means
> ...

6. Any other means
> ..

7. Our freedom
> ...


# Thanks
[Why choose actions as a changes handler ?](https://blog.andyet.com/2015/08/06/what-the-flux-lets-redux/)
